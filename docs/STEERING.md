...
**Why.** Two facts, from different places. The `ModelRepo` payload this
library reads from the unit contains `stepNames="Sine,Triang,...,Pink NS,White
NS"` verbatim. And Cortex Control fetches a `ModelRepo` payload once per session
in all three captures - 371 reports, gzip magic, ~46.7 KB. That CC therefore has
those strings is a strong inference rather than an observation; reassembling the
payload out of the pcapng would close it and was not achieved. An earlier draft argued this by elimination -
nothing else is big enough - which was false: `File` carries 885-report messages
against `ModelRepo`'s 371, and preset bodies carry `dynamic_steps`, which is
label text.
...
scale once this chain's own leakage is calibrated out. The catalog calls position 5
...
**The paragraph BELOW is superseded** by the second 2026-09-15 entry above,
written later the same day. Its conclusion - that presentation is not
