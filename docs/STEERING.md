...
- **Python >= 3.11.**
- **The environment is held to the pins, not just CI's.** `tests/test_packaging.py` compares every version installed in the running interpreter against the requirement that declares it, and skips what is not installed. CI installs from `pyproject.toml` and so always agrees; a working copy is installed by hand and drifted once - mypy 2.3.1 against a `<2` pin, for an unknown number of pull requests, which made every local "mypy clean" a claim about a checker CI does not run. Any pin refuses an install beneath its floor; only the `protobuf` and `mypy` pins can refuse a NEWER release, so upgrading anything else stays quiet however far it goes, and upgrading those two past their bound does not. `tests/test_packaging.py` holds this sentence to that set, in both directions - a pin that gains a ceiling and one that loses it.
...
- **Whether the mypy pin should allow 2.x.** It is `mypy>=1.15,<2`, and the ceiling's reason is not in an ADR. ADR-0018 put mypy in CI as a blocking job and says nothing about a bound; the bound arrived in b726d3e (2026-08-28), which says "mypy is pinned below 2 so the enforcer cannot change under CI" - the only place it was written down until this entry. It landed one day after ADR-0016 recorded that the static unit checking was VERIFIED with mypy 2.3.1 - the version the bound excludes, and the likeliest source of the drift this repo just found. mypy 2.3.1 was run against this tree on 2026-09-16 and both halves of CI's blocking job were clean: no issues in 46 source files, and `tests/test_typing.py` passed, so the unit checking still bites. That is one green run on one tree - it says raising the ceiling is open, not that it should be raised. The pin decides which checker every contributor runs, which makes it a change of its own. Note what trying it now costs: installing 2.x puts the environment outside the pin, so `tests/test_packaging.py` goes red until the pin moves with it. That is the check working, and it is the price of the experiment.
...
CI installs from `pyproject.toml`, so CI's blocking mypy job had been running a
different checker from the local one for an unknown number of pull requests, and
every local "mypy clean" was a claim about the wrong tool. Nothing turned out to
be broken - mypy 1.20.2 and 2.3.1 are both clean on this tree - but nothing in
the repo could have said so either way. The protobuf pin already has this shape
of guard against a committed artifact (ADR-0008); a pin with no artifact behind
it can only be checked against the environment, so that is what this does.

The venv was brought back inside the pin rather than the pin raised to admit
2.x. Raising it decides which checker every contributor runs, and the evidence
...
`changelog.md` since 0ae9102 (2026-08-27). The test still earns its place - nothing there
...
- **Updated:** `pyproject.toml` (a comment on the mypy pin and `packaging` added
  ...
  `changelog.md` (only the third runtime dependency, which is a user-facing
  fact; the pin check itself reaches no installed package);
...
- **Not updated (intentionally):** `ADR.md` - no decision was made or reversed.
  ...
  ceiling, whose reason lived only in b726d3e's commit message until this change
  copied it into `pyproject.toml` and section 8; that gap is written up there
  rather than backfilled into a record that did not make the call.
