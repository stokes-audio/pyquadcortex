...
| Per-beat accents (customizing each beat of the bar) | yes | `set_beat(n, MetronomeBeat.DOWN)` and `set_beats([...])`; `pyquadcortex.protocol.beats(preset)` reads them back. Tempo parameters 10-22 - the catalog's `STEPSTATE0` to `STEPSTATE12` - are beats 1 to 13, each a four-option list at `option / 3`. The device's own names for the four are `OFF`, `MUTE`, `DOWN`, `ON`, and they name the ACCENT rather than whether the beat sounds - `OFF` is the plain click and `MUTE` the silent one. Re-confirmed on the unit 2026-09-14. Traced by touching cells on the unit; a cell cycles UP by 1/3 and wraps, so four touches return it to where it started. Set the time signature FIRST - changing it rewrites these |
...
| Global EQ: bypass, 5 bands (type/gain/freq/Q/bypass), output assignment | yes | `set_global_eq(band, gain=, frequency=, q=, filter_type=, enabled=)`, `set_global_eq_output(level=, out12=, out34=)` and `set_global_eq_bypassed()`. Every control is reachable. `gain` takes `Db` over -12..+12, measured on screen at both ends (2026-09-11, CorOS 4.0.1), while `frequency`, `q` and the OUT level take `Encoded`, their mappings being unknown |
