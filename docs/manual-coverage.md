...
| Tempo value (per preset) | yes | `set_param(Tempo(), "TEMPO", real=120)` in bpm, or `value=` for the raw 0..1. The catalog names the bounds `MIN_TEMPO` / `MAX_TEMPO` and `steps=201` fixes them at whole bpm over 40..240; three INTERIOR screen readings (59, 111, 120 bpm) agree exactly |
