...
| Tap tempo | no | a `GlobalTempo` READ carries the 25 device tempo parameters, and none of them is a tap. MIDI CC#44 is the documented route |
| Tempo value (per preset) | yes | `set_tempo_param("TEMPO", real=120)` in bpm, or `value=` for the raw 0..1. The catalog range is a placeholder, so the 40..240 bpm span was measured off the screen instead - three points, exact to the displayed integer |
