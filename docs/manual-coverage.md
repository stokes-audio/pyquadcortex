...
| Favorites and Recents | partly | `recents()` reads the RECENTS list (name, folder key, folder name); the method was called `favorites()` until that was tested, and the old name is a deprecated alias. Both lists are maintained ONE ENTRY at a time with a `DELETE`+`CREATE` pair, captured from the unit's own traffic - an earlier "read-only" conclusion came from sending the whole list, which is simply the wrong shape. Favorites cannot be read on demand: asking for `is_favorites=True` draws no reply, and the list only turns up unbidden right after it changes, which is also why a host-side write of it cannot be verified |
