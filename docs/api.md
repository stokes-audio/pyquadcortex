...
| **Drive and inspect the unit's screen (`QuadCortex41`, CorOS 4.1.0)** | `tap_screen(x, y, timeout=)` taps one raw pixel coordinate; `swipe_screen(x, y, to_x, to_y, timeout=)` sends one bounded drag gesture; `capture_screen()` returns a PNG of the QC's current display; `graphics_tree()` returns its nonblank zenUI widget hierarchy. The 4.0.1 base exposes but refuses all four operations pending measurement |
