...
| **Synchronise the unit's screen (`QuadCortex41`, CorOS 4.1.0)** | `tap_screen(x, y)` taps one raw pixel coordinate; `capture_screen()` returns a PNG of the QC's current display (observed at 800 x 480). The 4.0.1 base exposes but refuses both operations pending measurement |
