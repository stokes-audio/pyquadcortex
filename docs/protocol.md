...
| `delete_setlist` (CorOS 4.1.0) | `File{DELETE, folder{key, is_factory: false}, delete_from_library: false}` | read-back | Cortex Control 4.1.0 shape measured on a disposable setlist 2026-09-11; destructive, so confirmation came from folder-listing read-back |
...
| `create_setlist` (CorOS 4.1.0) | `File{CREATE, folder{key, parent_key: "/media/p4/Presets", name, is_factory: false}}` | read-back | Cortex Control 4.1.0 shape measured on a disposable setlist 2026-09-11; the 4.0.1 shape above remains profile-local |
