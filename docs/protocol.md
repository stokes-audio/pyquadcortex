...
| `create_local_backup` | `LocalBackup{CREATE}` then ordered `LocalBackup{UPDATE, backup_json, is_last_chunk}` pushes | captured only | Contributed measurement 2026-09-08 on CorOS 4.1.0: 12 chunks (11 x 150,000 characters plus final 144,890), one final marker, no request id; joined wrapper and Base64 payload validated. Baseline 4.0.1 pending maintainer verification |
