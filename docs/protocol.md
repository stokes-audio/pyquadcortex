...
| `expression_assignments(preset)` | reads `params{expression, expression_min, expression_max}` across `models`, `output_control`, `input_control`, `mixer` and `combined_splitter` | fixture | the read-back side of `set_expression`, which had none. `params[].index` HAS NO PRESENCE - 0 for all 576 params in every committed fixture - so the POSITION is the index; `expression: 0` is unassigned, being what `clear_expression` writes |
