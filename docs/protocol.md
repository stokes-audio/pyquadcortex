...
| `preset_screenshot` (CorOS 4.1) | `Screenshot{READ, folder_name, is_factory, index}` | contributed hardware read-back | returns device-rendered PNG bytes; validates the echoed address and PNG signature. CorOS 4.0.1 refuses because this shape is unmeasured there |
