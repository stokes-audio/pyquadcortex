...
three `Version` messages - see section 4.) The ModelRepo landed at 4.9 s, the 399 `File`
listings and most settings at 5.1 s, and the seed
...
| `create_local_backup` | `LocalBackup{CREATE}` then one or more `LocalBackup{UPDATE, backup_json}` pushes, the last carrying `is_last_chunk` | captured only | Measured 2026-09-09 on Quad Cortex, CorOS 4.0.1 / d14e: a 130,178-character document arrived as ONE push, final marker set, nothing after it; no request id on the reply; the 10-field wrapper and its 64-hex `payload_hash` validated, and the Base64 payload decoded. 9.1 s on a cold connection, about 2.9 s after, and three back-to-back calls on one connection all succeeded. `can_apply_backup` never appeared, so the refusal path is unverified here. Contributed measurement 2026-09-08 on CorOS 4.1.0: 12 chunks (11 x 150,000 characters plus final 144,890), one final marker. Taken together, 150,000 is a MAXIMUM per push and not a fixed chunk size: a document under it arrives whole |
...
| `expression_assignments(preset)` | reads `params{expression, expression_min, expression_max}` across `models`, `output_control`, `input_control`, `mixer` and `combined_splitter` | fixture | the read-back side of `set_expression`, which had none. `params[].index` is proto3-optional so it HAS presence, and the device never sends it - absent on all 576 params in every committed fixture - so the POSITION is the index. `expression: 0` is unassigned, being what `clear_expression` writes, and IS sent. NOT walked, neither established: `chain.splitter` (a different model from `combined_splitter` - 10002 with 3 params against 10004 with 8) and `Tempo`, which has no row and sits outside `chains` |
